...
abstract class DHLParcel_Shipping_Model_Data_AbstractData
{
    public function __construct($automap = [])
    {
                ...
                if (is_array($array[$key])) {
                    if (array_key_exists($key, $classArrayMap)) {
                        ...
                        foreach ($array[$key] as $entry) {
                            if ($entry instanceof DHLParcel_Shipping_Model_Data_AbstractData) {
                                $this->{$key}[] = $entry;
                            } else {
                                $this->{$key}[] = Mage::getModel($class, $entry);
                            }
                        }
                    } elseif (array_key_exists($key, $classMap)) {
                        // Class mapper
                        $class = $classMap[$key];
                        $this->$key = Mage::getModel($class, $array[$key]);
                    } else {
